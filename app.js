...
document.querySelector('input').addEventListener('keyup', function() {
    SecurityKey = true
    if (document.querySelector('input').value === randomWord.name) {
        if (Score % 10 == 0) {
            addStage()
        }
...
function setRandomWord() {
    var ContriesAPI = "https://restcountries.eu/rest/v2/all";

    $.getJSON(ContriesAPI, function(data){
        
        Words = data
        randomWord = Words[Math.floor(Math.random()*Words.length)]
        $('.word').html(randomWord.name)
    }).fail(function(){
